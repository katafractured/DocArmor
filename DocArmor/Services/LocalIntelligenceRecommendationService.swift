...
enum LocalIntelligenceRecommendationService {
    ...
    static func packRecommendations(
        ...
        let recommendations: [PackRecommendation?] = [
            ...
            enabledPacks.contains(.school) ? nil : pack(.school, "School Pack", "graduationcap.fill", roles.contains(.child) ? "A child profile makes school and enrollment access relevant." : nil),
            ...
            enabledPacks.contains(.dependent) ? nil : pack(.dependent, "Dependent Care Pack", "person.2.crop.square.stack.fill", roles.contains(where: { $0 == .child || $0 == .senior }) ? "Child or senior profiles are present and benefit from a dedicated care packet." : nil),
